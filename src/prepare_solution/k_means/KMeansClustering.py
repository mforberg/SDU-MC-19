...
def k_means_clustering(amount_of_clusters, list_of_starting_points):
    ...
    # wcss = []
    # # max_cluster = 11
    # # if len(list_of_starting_points) < max_cluster:
    # #     max_cluster = len(list_of_starting_points)
    # # print max_cluster, "max"
    # for i in range(1, 8):
    #
    #     kmeans = KMeans(n_clusters=i, init="k-means++")
    #     kmeans = kmeans.fit(list_of_starting_points)
    #     kmeans.predict(list_of_starting_points)
    #     wcss.append(kmeans.inertia_)
    #     print(kmeans.inertia_)
    # p1 = (1, wcss[0])
    # p1 = np.asarray(p1)
    # p2 = np.asarray(10, wcss[6])
    # p2 = np.asarray(p2)
    #
    # for i in range(0, 7):
    #     p3 = (i + 1, wcss[i])
